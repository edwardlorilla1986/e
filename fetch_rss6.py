...
def insert_blog_post_to_db(title, summary, content, keywords, slug, thumbnail):
    ...
    cursor.close()
    db.close()
rss_url = 'https://tempo.com.ph/feed/'
    ...
    for entry in existing_entries:
        ...
        try:
            ...
            if "Error" not in blog_content:
                ...
                send_email(
                    recipient_email="edwardlorilla2206.edwardlancelorilla@blogger.com",
                    subject=blog_content["title"].replace('The title of the blog post is', '').replace(':', '').replace('<br>', '').replace('*', '').replace('The title of this edited blog post is', '').replace('Based on your edited blog post, I would title it:', ''),
<<<<<<< HEAD
